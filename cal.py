...
class Calculator:
    def __init__(self,master):
        ...
        master.geometry('357x400+0+0')
        master.config(bg='red')
        master.resizable=(False,False)

        self.equation=StringVar()
        self.entry_val=''
        Entry(width='20',bg='white',font=('Arial ',28),textvariable=self.equation).place(x=0,y=0)
        
        Button(width=11,height=4,text='(',relief='flat',bg='white',command=lambda:self.show('(')).place(x=0,y=50)
        Button(width=11,height=4,text=')',relief='flat',bg='white',command=lambda:self.show(')')).place(x=90,y=50)
        Button(width=11,height=4,text='%',relief='flat',bg='white',command=lambda:self.show('%')).place(x=180,y=50)
        Button(width=11,height=4,text='AC',relief='flat',command=self.clear).place(x=270,y=50)
